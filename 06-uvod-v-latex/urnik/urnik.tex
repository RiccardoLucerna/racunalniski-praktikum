...
\begin{itemize}
   ...
   \item Razred \emph{Predavalnica} nosi podatke o kapaciteti predavalnice, njeno oznako ter podatke o zasedenosti, ki so predstavljeni s tabelo.
...
$$\texttt{velikostTabele} = \texttt{številoUrNaDan}\cdot\texttt{številoDelovnihDni}$$
...
\begin{enumerate}
   ...
   označimo s $\texttt{predmet}_i$.

   \item Naslednja vgnezdena zanka nas popelje po vseh predavalnicah. Predavalnico na
   $j$-tem koraku označimo s $\texttt{predavalnica}_j$.

   \item Za vse proste termine $\texttt{predavalnice}_j$ izračunamo ceno vstavljanja
   $\texttt{srečanja}_i$, ki ga sestavlja $\texttt{predmet}_i$, profesor, ki ta predmet predava,
   ...
   vstavljanja, da bomo $\texttt{predmet}_i$ na koncu vstavili v termin z najboljšo
   ceno vstavljanja.

   \item Ko preverimo vse proste termine vseh predavalnic, vstavimo $\texttt{srečanje}_i$
...
spremembo (npr.\ zamenjava $\texttt{srečanja}_i$ in $\texttt{srečanja}_j$) sedaj shranimo v
